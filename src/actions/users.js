...
export function register(registerData) {
  return (dispatch) => {
    ...
    const url = `${BASE_URL}/${
      registerData.status === "buyer" ? "buyer" : "seller"
    }/sign-up`;
    ...
    axios
      ...
      .then((response) => {
        ...
        localStorage.setItem(
          "shippingAddress",
          JSON.stringify(response.data.shippingAddress)
        );
...
export function getUnApprovedSellers() {
  return (dispatch) => {
    ...
    axios
      ...
      .then((response) =>
        dispatch(fetchingUnapprovedSellerListFetchDataSuccess(response.data))
      )
